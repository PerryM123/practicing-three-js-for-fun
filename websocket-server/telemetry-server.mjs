...
setInterval(() => {
  ...
  const wobbleX =
    5 * Math.sin((nowMs / 1000) * 0.5 + wobbleSeedX + phaseIndex * 0.37)
  const wobbleZ =
    5 * Math.cos((nowMs / 1000) * 0.5 + wobbleSeedZ + phaseIndex * 0.41)
  ...
  currentBodyRotation = lerp(
    currentBodyRotation,
    targets.bodyRotation,
    LERP_FACTOR
  )
  currentArmAngle = lerp(currentArmAngle, targets.armAngle, LERP_FACTOR)
  currentBucketDepth = lerp(
    currentBucketDepth,
    targets.bucketDepth,
    LERP_FACTOR
  )
